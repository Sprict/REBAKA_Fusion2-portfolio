...
撮影・編集済みの実演動画を以下の順で掲載します（2026-07-13時点でアップロード作業中。準備でき次第リンクを追加します）。

1. **まず見る — プレイヤーアクション一覧**: 移動・ジャンプ・しゃがみ・掴み・運搬などの操作を一通り実演
2. **ネットワーク実機検証 — 応答遅延テスト**: Windows（ホスト）と Mac（クライアント）の実機2台を1画面に収め、ゲームパッド入力→ホスト反映→クライアント反映の遅延を実撮影
3. **切断耐性 — 同一PC2クライアントテスト（ノーカット）**: 参加→切断→再参加→ホスト終了時のクライアント安全復帰→ホスト不在時のJoin失敗処理までを編集なしで収録
4. **自動生成・同期 — マップ生成の2クライアント同期**（テロップ作業中）: シード値を変えた3例で、ホストが生成したマップがクライアントに同一再構築される様子
5. **開発過程 — 2020→2026 開発変遷**（テロップ作業中）: 前身プロジェクトの物理発散から現在の安定同期までの記録映像

### 🕹 実行ファイル（プレイアブルビルド）

Windows / macOS のプレイアブルビルド（テストシーン）を用意しています。オンライン接続用の認証情報（Photon App ID）を含むため一般公開はせず、**選考ご担当者様には応募時に個別提供いたします**（Windows 版を推奨。macOS 版は未署名のため初回起動時に手動許可が必要です）。
