...
- 更新頻度は低くてよい。**1つの大きな機能がまとまった区切り**（新しいクラス群の追加、同期方式の変更など）が来たときに見直す。
- 毎回のコミットに追従させる必要はない。
- **AIエージェントへの指示: このファイルを更新する前に、必ずユーザーへ「今更新してよいか」を確認すること。** トークン残量の都合で今は更新したくない、というケースがあるため、無断で書き換えない。
- 個別の変更理由・調査プロセスは devlog（`Docs/devlogs/`）に書く。このファイルには残さない。
