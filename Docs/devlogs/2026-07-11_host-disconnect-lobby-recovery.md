...
Fusion 2 の `NetworkRunner` は Shutdown 後に再利用できない。設計書（`docs/superpowers/specs/2026-07-10_host-disconnect-recovery-design.md`）では停止済み Runner 参照を保持して次回 `StartSession` で `DestroyImmediate` する案もあったが、今回は **`Test_Playground` を Single で再読み込み**し、Editor 配置の `Runner` GameObject（SessionManager / PlayerSpawner 含む）を丸ごと復元する方式を採用した。
...
- 設計ドラフト: `docs/superpowers/specs/2026-07-10_host-disconnect-recovery-design.md`（実装はシーン reload 方式に寄せている）
