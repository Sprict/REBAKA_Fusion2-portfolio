# クライアント側ラグドール表示の商用品質化: 全身ポーズ・スナップショット補間方式

日付: 2026-06-10
ブランチ: feature/concept-v3-spec
関連: BN-4（ハードスナップ）、BN-6（四肢ドリフト）、2026-04-11 ボトルネック分析、2026-03-27 Forecast A/B テスト
...
3. **同期は Root/Head/両手の4パーツのみ**（BN-6）— 残り11パーツはクライアントローカル物理
   （ジョイント駆動）で動かしており、PhysX の非決定論性により発散。
   発散が 1m を超えるとハードスナップ（瞬間テレポート、BN-4）が発生していた。
...
- コンパイル: エラー 0 / 警告 0
- ホスト退行確認（uloop 自動化）: スポーン (-3.90, 0.89, -4.42) → W 4 秒で
