...
姉との2クライアント実機テスト（Windows PC=ホスト、MacBook Pro 2018=クライアント）で以下を確認:
...
`RagdollProfile.carryLegDriveMaxForce`（Treasure保持中に脚のJointDrive最大力を制限する仕様）が存在したが、ユーザーはこれを指示した記憶がないと明言。実装計画書（`Docs/superpowers/plans/2026-04-26-mvp-physical-carry.md`）にもこの仕様の記載はなく、2026-07-03の大規模チェックポイントコミット（`c839815`、ポーズオーサリング等の無関係な変更と一括コミット）に紛れて導入されていた。経緯が追えないため、AIが要件を拡大解釈して自律的に追加した可能性が高いと判断し、撤去した。
...
- 全ての変更は静的読解と力学的な見積もりに基づく仮説であり、**2クライアント実機テストでの検証が必須**（`.claude/rules/network.md` の必須検証ルール）
