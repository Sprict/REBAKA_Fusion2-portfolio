# 開発ログ: 2026-02-18 - クライアントスポーン直後KO問題とState Authorityガード

> **公開時注記（2026-07-22）:** これは2026年2月時点の旧実装を記録した資料です。State Authorityだけが本体物理を確定する判断は現行方式にも引き継がれていますが、クラス構成とコード例は後に変更されています。「Gang Beasts方式」は当時使用した通称であり、同作品の実装を一次資料で確認した呼称ではありません。現在の構成は[`ARCHITECTURE_OVERVIEW.md`](../ARCHITECTURE_OVERVIEW.md)を参照してください。
...
**当時の`GetInput()`と`HasStateAuthority`ガードの順序:**
...
当時のコードでは`GetInput()`を呼んだ後に、State Authority以外の物理処理を止めていた。ただし、
`GetInput()`の呼び出し自体が入力を「受信済み」としてマークする、という説明は一次資料で確認できていない。
現在の実装構成も異なるため、この順序をFusion全般の必須ルールとしては扱わない。
...
**本資料内での呼称について**: 当時はホスト権威型の物理構成を「Gang Beasts方式」と呼んでいたが、
同作品の実装を一次資料で確認したものではない。現在の公開資料では「State Authorityのみが本体物理を確定し、
クライアントは確定ポーズを補間表示する方式」と記述する。
...
- [ ] 当時の実装で`GetInput()`を先に呼んでいた理由と、現在はFusion全般の必須ルールとして扱っていないことを区別する
