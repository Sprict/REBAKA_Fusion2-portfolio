...
- 関連: [[project-peer-sync-pure-interpolation]] / 2026-06-18_devlog.md（スポーン位置同期の前段修正）
...
- **(却下) Forecast Physics（外挿）方式**: 2026-03-28 の A/B テストで kinematic+補間方式に全敗済み
  （[[project_forecast_physics_decision]]）。
...
- **weaver 安全性**: `[Networked]` プロパティと `[Rpc]` を**一切追加しない**ため、基底の
  ...
  `uloop compile` で 0 errors / 0 warnings を確認済み（= weaver が受理）。
...
- `uloop compile --wait-for-domain-reload true` → **0 errors / 0 warnings**。
- Unity 上で両プレハブが `MyFolder.Scripts.Network.GameNetworkRigidbody` に解決、Missing Script なし
  （`execute-dynamic-code` で確認）。
...
## 7. その他の残課題・フォロー
- **Treasure 系の同期方式**: `Treasure_Heavy` は `NetworkRigidbody` 非依存の独自同期。Treasure でも隙間や
  同種問題が出るなら別途同方式へ寄せる検討が要る（今回スコープ外）。
- スポーン時に「投げられた（移動中の）状態」で出現する Cube のエッジは未検証（静止前提で動作）。
- **致命的な前提（別 memory 参照）**: Fusion の weave は stray な重複 NetworkProjectConfig（`Assets/Photon`）で
  全滅する。ParrelSync clone は Library 独自のため各自再コンパイルが必要。詳細は
  `[[project-fusion-stray-networkprojectconfig]]`。
