...
> **公開時注記（2026-07-22）:** この削除は現行コードへ反映済みです。本資料は、旧同期方式の設定・class・参照を依存単位で確認して削除した記録であり、現在の同期方式そのものの説明ではありません。
...
- 現行のプロキシ同期は **SnapshotInterpolation（全パーツkinematic + `Render()` での純視覚補間）** に確定済み。Hybrid方式は `RagdollSyncMode` のenumに参考用として名前が残るだけで、PIDゲインを読み出す実コードは存在しない。
...
- [ ] UnityでDomain Reloadを伴う再コンパイルを行い、エラー0を確認。
