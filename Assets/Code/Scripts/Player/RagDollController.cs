...
namespace MyFolder.Scripts.Player
{
    ...
    public class RagdollController : NetworkBehaviour, IBeforeTick,
        ...
        IRagdollAudioSink, IRagdollGroundingSink, ILocalPlayerViewSource, IProxyPoseSource, IPoseSnapshotAccess
    {
        ...
        public override void Spawned()
        {
            try
            {
                ...
                {
                    Debug.LogError(
                        $"bodyRigidbodies には、ポーズ同期対象の Rigidbody をちょうど {expectedBodyCount} 個登録してください " +
                        $"（現在: {bodyRigidbodies?.Length ?? 0} 個）。" +
                        "装飾用の Rigidbody（Other / Sphere など）は、ここに登録してはいけません。" +
                        "2026-06-12 の prefab 修正を取得した後にこのエラーが出た場合、Editor が古い import 情報を保持している可能性があります。" +
                        "newAPRPlayer.prefab を再インポートするか、Editor を再起動してください。" +
                        "注意: Console の [Error Pause] を有効にしていると、このエラー発生時にシミュレーション全体が停止します。",
                        this);
                }
        ...
        bool IClientProxyRuntimeContext.TryGetInput(out NetworkInputData inputData)
        {
            return GetInput(out inputData);
        }
        ...
        bool IHostSimulationContext.TryGetInput(out NetworkInputData inputData)
        {
            return GetInput(out inputData);
        }
