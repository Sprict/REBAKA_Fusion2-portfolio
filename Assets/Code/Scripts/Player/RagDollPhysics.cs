...
namespace MyFolder.Scripts.Player
{
    ...
    public class RagdollPhysics
    {
        ...
        public void UpdatePhysics(PlayerState state, RagdollCommand command, float deltaTime)
        {
            ...
            switch (state)
            {
                ...
                case PlayerState.Walking:
                    ApplyMovementForce(command.MoveDirection, deltaTime, movementControlMultiplier);
                    if (!_isRagdoll)
                        ProcessWalking(command.MoveDirection, deltaTime);
                    break;
        ...
        private void ApplyMovementForce(Vector3 moveDirection, float deltaTime, float controlMultiplier)
        {
            ...
            IRagdollTreasureCarryContext carryContext = _context as IRagdollTreasureCarryContext;
            if (carryContext != null && carryContext.IsGrabbingTreasure)
            {
                Vector3 currentVelocity = rb.linearVelocity;
                Vector3 currentHorizontalVelocity = new Vector3(currentVelocity.x, 0f, currentVelocity.z);
                Vector3 targetHorizontalVelocity = new Vector3(targetVel.x, 0f, targetVel.z);
                Vector3 deltaVelocity = targetHorizontalVelocity - currentHorizontalVelocity;
                if (deltaTime <= 0f)
                    return;

                Vector3 requiredForce = rb.mass * deltaVelocity / deltaTime;
                float maxForce = Mathf.Max(0f, carryContext.CarryMoveMaxForce);
                rb.AddForce(Vector3.ClampMagnitude(requiredForce, maxForce), ForceMode.Force);
                return;
            }
