    ...
    public sealed class SettingsMenuController : MonoBehaviour
    {
        ...
        private void WireTabs()
        {
            ...
            for (int i = 0; i < tabs.Length; i++)
            {
                ...
                // タブごとに Down → firstFocus を固定（タブ増減は配列要素の追加だけで足りる）
                if (tab.firstFocus != null)
                    SetSelectOnDown(tab.tabButton, tab.firstFocus);
            }
        }
        ...
        private static void SetSelectOnDown(Selectable selectable, Selectable downTarget)
        {
            if (selectable == null)
                return;
